...
class ToronitesHelper:
    ...
    def get_season_number(self, slug) -> str:
        season_str = slug
        season_str = season_str.replace("\n", " ").lower()
    ...
    def generate_film_data(
        ...
        for info_key in key_mapping.keys():
            if info_key in extra_info.keys():
                if not post_data.get(key_mapping[info_key], ""):
...
class Toronites:
    ...
    def insert_movie_details(self, post_id):
        ...
        for ep_num, ep_data in self.episodes.items():
            server_name = ep_data.get("name", CONFIG.LINK_LANGUAGE)
            link = ep_data.get("episodes", {}).get("movie_episode", "")
            postmeta_data.append(
                (
                    post_id,
    ...
    def format_episodes(self):
        res = {}
        for episode_number, episode_data in self.episodes.items():
            episodes = episode_data.get("episodes", {})
            name = episode_data.get("name", CONFIG.LINK_LANGUAGE)
            for ep_title, ep_link in episodes.items():
                res.setdefault(ep_title, [])
                res[ep_title].append([name, ep_link])

        self.episode = res

    def insert_episode(self, post_id: int, season_term_id: int, thumb_id: str = "0"):
        self.format_episodes()
        len_episodes = 0

        for ep_title, ep_links in self.episode.items():
            episode_title_self_created = self.film["post_title"] + f"x{ep_title}"
            episode_term_name = ep_title

            episode_term_slug = slugify(self.film["slug"] + f"x{ep_title}")
            ...
            len_episode_links = 0
            logging.info(f"Inserting new Episode {ep_title}")

            try:
                episode_number = ep_title.split(":")[0].strip()
                episode_number = int(episode_number)
            except Exception as e:
                helper.error_log(
                    msg=f"Error getting episode number: \n{self.film.get('slug', '')} {ep_title}",
                    log_file="toronites.get_episode_number.log",
                )
                continue

            termmeta_data = [
                (episode_term_id, "episode_number", episode_number),
                (episode_term_id, "name", ep_title),
    ...
    def insert_season(self, post_id: int):
        ...
        season_term_slug = self.film["slug"]
    ...
    def insert_film(self):
        ...
        self.film["season_number"] = helper.get_season_number(self.film["slug"])
