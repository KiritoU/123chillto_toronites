...
class Helper:
    ...
    def get_trailer_id(self, soup: BeautifulSoup) -> str:
        try:
            youtube_id_tv = soup.find("div", class_="youtube_id_tv")
            iframe = youtube_id_tv.find("iframe")
            data_src = iframe.get("src")
            trailer_id = data_src.strip("/").split("/")[-1]
            return trailer_id
        except:
            return ""
    ...
    def get_title(self, href: str, mvi_content: BeautifulSoup) -> str:
        try:
            heading_name = mvi_content.find("h3").text.strip("\n")
            return heading_name
        except Exception as e:
            self.error_log(
    ...
    def get_cover_url(self, href: str, mvi_content: BeautifulSoup) -> str:
        try:
            dm_thumb = mvi_content.find("div", class_="dm-thumb")
